...
let $carrosel = $('#carroussel'), // on cible le bloc du carrousel
    $img = $('#carroussel img'), // on cible les images contenues dans le carrousel
...
function newImg(){ //fonction de remplacer l'ancienne image par une nouvelle image
...
function start(){ //fonction lançant l'autoslider
  intervalId = setInterval(newImg, 2000); //attend 2 secondes avant de lancer la fonction newImage puis se répète indéfiniment
}


printImg();//on affiche la premiere image

$("#bouton1").click(function(){ // image précédente
...
$("#bouton2").click(function(){ // image suivante
    if(i < indexImg){ // si le compteur est inférieur au dernier index
        i++; // on l'incrémente
